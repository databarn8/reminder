...
def direct_mode():
    ...
    file_pattern = input("Enter the file path to upload (supports wildcards like *.jpeg or partial names): ").strip()
    
    # Expand relative paths
    file_pattern = os.path.expanduser(file_pattern)
    
    # Check if it's an exact file that exists
    if os.path.isfile(file_pattern):
        file_to_upload = file_pattern
        print(f"✓ Found file: {file_to_upload}")
    else:
        # Treat as pattern or partial name
        matching_files = []
        
        # First try as-is if it has wildcards
        if '*' in file_pattern or '?' in file_pattern:
            matching_files = glob.glob(file_pattern, recursive=False)
            
            # If no matches, try recursive search
            if not matching_files and '/' not in file_pattern and '\\' not in file_pattern:
                matching_files = glob.glob(f"**/{file_pattern}", recursive=True)
        else:
            # No wildcards - search for files containing this text
            ...
            while True:
                try:
                    ...
                    if 0 <= idx < len(matching_files):
                        file_to_upload = matching_files[idx]
                        break
                    else:
                        print("Invalid selection. Try again.")
                except ValueError:
    ...
    print(f"\n📤 Uploading '{os.path.basename(file_to_upload)}'...")
