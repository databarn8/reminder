...
class NotificationScheduler : BroadcastReceiver() {
    ...
    companion object {
        ...
        fun scheduleReminder(context: Context, reminder: Reminder) {
            ...
            // FIX: Only schedule primary alarm (AT_DUE_TIME) to prevent multiple alarms for same reminder
            // This prevents visual repetition issue where same reminder appears multiple times
            val primaryTriggerPoint = triggerPoints.find { it.type == com.reminder.app.data.TriggerType.AT_DUE_TIME }
                ?: triggerPoints.firstOrNull() // Fallback to first trigger point if AT_DUE_TIME not found
            
            primaryTriggerPoint?.let { triggerPoint ->
                val triggerTime = triggerPoint.calculateTriggerTime(reminder.reminderTime)
                val currentTime = System.currentTimeMillis()
                
                // Only schedule if trigger time is in future
                if (triggerTime > currentTime) {
                    ...
                    // Use unique request code for primary trigger
                    val requestCode = "${reminder.id}_primary".hashCode()
                    ...
                    android.util.Log.d("NotificationScheduler", "Primary trigger: ${triggerPoint.getDescription()}")
                    ...
                    try {
                        ...
                        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                            if (!alarmManager.canScheduleExactAlarms()) {
                                android.util.Log.w("NotificationScheduler", "Cannot schedule exact alarms - permission missing")
                                ...
                                android.util.Log.d("NotificationScheduler", "Primary alarm scheduled with setAndAllowWhileIdle (fallback)")
                            } else {
                                ...
                                android.util.Log.d("NotificationScheduler", "Primary alarm scheduled with setExactAndAllowWhileIdle (API 31+)")
                            }
                        } else {
                            ...
                            android.util.Log.d("NotificationScheduler", "Primary alarm scheduled with setExact (pre-API 31)")
                        }
                        
                        // Verify alarm was set
                        android.util.Log.d("NotificationScheduler", "Primary alarm verification - Request code: $requestCode, Target time: ${java.util.Date(triggerTime)}")
                        
                    } catch (e: SecurityException) {
                        android.util.Log.e("NotificationScheduler", "SecurityException for primary alarm: ${e.message}")
                        try {
                            alarmManager.setAndAllowWhileIdle(
                                AlarmManager.RTC_WAKEUP,
                                triggerTime,
                            ...
                            android.util.Log.d("NotificationScheduler", "Primary alarm scheduled with setAndAllowWhileIdle (security fallback)")
                        } catch (e2: Exception) {
                            android.util.Log.e("NotificationScheduler", "Final fallback to set for primary alarm: ${e2.message}")
                            try {
                                alarmManager.set(
                                    AlarmManager.RTC_WAKEUP,
                                    triggerTime,
                                ...
                                android.util.Log.d("NotificationScheduler", "Primary alarm scheduled with set (inexact fallback)")
                            } catch (e3: Exception) {
                                android.util.Log.e("NotificationScheduler", "All alarm scheduling methods failed for primary alarm: ${e3.message}")
                                e3.printStackTrace()
                            }
                        }
                    } catch (e: Exception) {
                        android.util.Log.e("NotificationScheduler", "Failed to schedule primary alarm: ${e.message}")
                        e.printStackTrace()
                    }
                } else {
                    android.util.Log.d("NotificationScheduler", "Skipping primary trigger - time is in the past")
                }
            } ?: run {
                android.util.Log.w("NotificationScheduler", "No valid trigger points found for reminder ${reminder.id}")
            }
        ...
        private fun scheduleRepeatAlarms(context: Context, reminder: Reminder) {
            ...
            // FIX: Only schedule the next 2 occurrences to prevent notification spam
            // This prevents multiple notifications for the same reminder
            val nextOccurrences = reminder.getFutureOccurrences(2)
            
            // Schedule only the next 2 future occurrences
        ...
        private fun showNotification(context: Context, title: String, content: String, reminderId: Int) {
            ...
            // Also create regular intent for notification tap
            val regularIntent = Intent(context, MainActivity::class.java).apply {
                flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK
            }
    
            val pendingIntent = PendingIntent.getActivity(
                context,
                reminderId + 1000, // Different code to avoid conflicts
                regularIntent,
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
            )
            ...
            val notification = NotificationCompat.Builder(context, CHANNEL_ID)
                ...
                .setFullScreenIntent(fullScreenPendingIntent, true) // This should trigger AlarmActivity
        ...
        private fun launchAlarmActivity(context: Context, title: String, content: String, reminderId: Int, reminder: Reminder? = null) {
            try {
                ...
                // For Android 11+, use a different approach to start activity from background
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
                    // Create a pending intent with high priority
                    val pendingIntent = PendingIntent.getActivity(
                        context,
                        reminderId,
                        intent,
                    ...
                    try {
                        pendingIntent.send()
                        android.util.Log.d("NotificationScheduler", "AlarmActivity launched via PendingIntent for reminder: $title")
                    } catch (e: Exception) {
                        android.util.Log.e("NotificationScheduler", "Failed to send PendingIntent: ${e.message}")
                        // Fallback to regular startActivity
                        context.startActivity(intent)
                        android.util.Log.d("NotificationScheduler", "AlarmActivity launched via fallback for reminder: $title")
                    }
                } else {
                    // For older Android versions, use regular startActivity
                    context.startActivity(intent)
        ...
        private fun createNotificationChannel(context: Context) {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                val channel = NotificationChannel(
                    ...
                    // For Android 11+, enable full-screen intents
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
                        setBypassDnd(true)
                        enableLights(true)
                        enableVibration(true)
                        // This is critical for Android 11+ to allow full-screen activities
                        lockscreenVisibility = android.app.Notification.VISIBILITY_PUBLIC
                    }
                }
