            ...
            FileOutputStream(file).use { fos ->
                val csvContent = buildString {
                    ...
                    reminders.forEach { reminder ->
                        val alertLevel = try {
                            val alertLevelField = reminder.javaClass.getDeclaredField("alertLevel")
                            alertLevelField.isAccessible = true
                            alertLevelField.get(reminder) as? String ?: "LOW"
                        } catch (e: Exception) {
                            "LOW"
                        }
                        appendLine("${reminder.id},\"${reminder.content}\",\"${reminder.category}\",${reminder.importance},${reminder.reminderTime},${reminder.createdAt},$alertLevel,${reminder.repeatType}")
                    }
                }
