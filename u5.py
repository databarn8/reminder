...
def direct_mode():
    ...
    file_pattern = input("Enter the file path to upload (supports wildcards like *.jpeg): ").strip()
    
    # Expand relative paths
    file_pattern = os.path.expanduser(file_pattern)
    
    # Check if it's a wildcard pattern or exact path
    if '*' in file_pattern or '?' in file_pattern:
        # Use glob to find matching files
        matching_files = glob.glob(file_pattern, recursive=False)
        
        # If no matches with the pattern as-is, try recursive search
        ...
        if os.path.isfile(file_pattern):
            ...
            # Try as a pattern anyway (in case the user forgot wildcards)
